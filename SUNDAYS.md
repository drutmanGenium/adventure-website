...
│   ├── trekking-detail.tsx       # Full trekking detail page (itinerary, pricing, gallery, JSON-LD structured data)
...
| `components/trekking-detail.tsx` | Trekking detail page with itinerary, gallery, pricing, date selection, and JSON-LD structured data (`TouristTrip` + `BreadcrumbList` schema) injected via `next/script` |
...
**8. JSON-LD Structured Data on Trekking Detail Pages**

The `trekking-detail.tsx` component generates JSON-LD structured data for each trekking page to improve search engine visibility. The structured data is computed via `useMemo` and injected into the page using `next/script` with `type="application/ld+json"`. The JSON-LD uses a `@graph` array containing two schema types:

- **`TouristTrip`** -- includes the trekking name, description, type, URL, image, difficulty (mapped to schema.org difficulty levels), itinerary as an `ItemList`, pricing as an `Offer` (derived from the linked `Activity` data), provider info for "Patagonia Trek", and location.
- **`BreadcrumbList`** -- three-level breadcrumb: Inicio > Actividades > [Trekking Title].

```tsx
// components/trekking-detail.tsx
const jsonLd = useMemo(() => {
  ...
  const graph: Record<string, unknown>[] = [
    {
      ...
      "itinerary": {
        "@type": "ItemList",
        "itemListElement": trekking.itinerary.map((step, i) => ({
          "@type": "ListItem",
          "position": i + 1,
      ...
      "provider": { "@type": "TourOperator", "name": "Patagonia Trek", /* ... */ },
      "location": { "@type": "Place", "name": trekking.location, /* ... */ },
    },
  ]
  ...
  graph.push({
    "@type": "BreadcrumbList",
    "itemListElement": [
      { "@type": "ListItem", "position": 1, "name": "Inicio", "item": "https://patagoniatrek.com.ar" },
      { "@type": "ListItem", "position": 2, "name": "Actividades", "item": "https://patagoniatrek.com.ar/actividades" },
      { "@type": "ListItem", "position": 3, "name": trekking.title, "item": `https://patagoniatrek.com.ar/trekkings/${id}` },
    ],
  })
...
The script tag is rendered conditionally at the top of the component return, using a fragment wrapper:
...
- The `Offer` block is only included when `activityData` exists (i.e., the trekking ID matches an entry in the `ACTIVITIES` array from `actividades-view.tsx`).
- Availability is set to `InStock` or `SoldOut` based on whether there are future departure dates.
- The `validFrom` field on the offer uses the earliest future date.
- Difficulty values are mapped from Spanish labels ("Fácil", "Moderado", "Avanzado") to schema.org URIs. Unrecognized difficulty values are silently omitted from the structured data.
- The canonical URL base is hardcoded to `https://patagoniatrek.com.ar`.
...
| `/trekkings/[id]` | `TrekkingDetailPage` | Dynamic detail page for a specific trekking (itinerary, gallery, pricing) with JSON-LD structured data for SEO |

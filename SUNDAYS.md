...
| `components/trekking-detail.tsx` | Trekking detail page with itinerary, gallery, date picker, pricing, and JSON-LD structured data (`TouristTrip` + `Product` schema.org types) |
...
The `trekking-detail.tsx` component injects JSON-LD structured data into each trekking detail page for SEO. It uses the Next.js `Script` component (from `next/script`) to render a `<script type="application/ld+json">` tag containing a schema.org `@graph` with two entities:

- **`TouristTrip`** -- describes the trekking as a tourist activity, including the itinerary as an `ItemList`, the provider as a `TravelAgency` (Patagonia Trek), and the location as a `Place` with a `PostalAddress` in Ushuaia.
- **`Product`** -- describes the trekking as a purchasable product with an `Offer` (price from `activityData.price_from` and `activityData.currency`) and an optional `AggregateRating` (when `activityData.rating` is present).

The structured data is built dynamically from the trekking record and the matching `ACTIVITIES` entry:
...
// Rendered as the first child inside the component's root <div>:
<Script
...
The `activityData` variable is the matching entry from the `ACTIVITIES` array (imported from `actividades-view.tsx`), looked up by `id`. When `activityData` is not found (e.g., a trekking with no corresponding activity listing), the `Product` entity omits the `offers` and `aggregateRating` fields. This pattern complements the existing JSON-LD in `lugar-landing.tsx`, which uses `TouristAttraction` and `FAQPage` schemas for destination pages.
...
| `/trekkings/[id]` | `TrekkingDetailPage` | Dynamic detail page for a specific trekking (itinerary, gallery, pricing, JSON-LD structured data) |
